...
function loadTemplateColumns(end){
  ...
  var mongodb = require('mongojs').connect("files", [collection]);
...
function saveToSQLite(end) {
  ...
  sqldb = new sqlite3.Database(sqlitefile, function(err){
    ...
    sqldb.serialize(function() {
      ...
      sqldb.run("CREATE TABLE " + collection + " (" + columns.join(',') + ")");

      for(d in data){
        var object = data[d];
        var row = [];
        for(c in columns){ row.push(JSON.stringify((object[columns[c]]||'').toString().replace(/[^a-zA-Z0-9 ]/gi,''))); }
...
exports.update = function (MongoDBCollectionName, SQLiteFile, callback){
