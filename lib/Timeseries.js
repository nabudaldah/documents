...
Timeseries.prototype.toCSV = function(separator, format){
    ...
    var unit  = this.interval.match(/m|h|d/).toString();
    ...
    while(iter.hasNext()) time.push(iter.next().format(format));
...
Timeseries.prototype.test = function(){
  ...
  if(!moment("2013-01-04T00:05:00+01:00").isSame(ts.roundUp(date))){
    console.error('Timeseries.prototype.roundUp(): failed test: returned ' + ts.roundUp(date) + ' while expected ' + '2013-01-04T00:05:00+01:00');
  };
  ...
  if(!moment("2013-01-04T00:01:00+01:00").isSame(ts.roundDown(date))){
    console.error('Timeseries.prototype.roundDown(): failed test: returned ' + ts.roundDown(date) + ' while expected ' + '2013-01-04T00:01:00+01:00');
  };

  /*Timeseries.prototype.toCSV() */
  var base = "2014-01-01T00:00:00+01:00";
  var interval = '15m';
  var ts = new Timeseries({base:base, interval:interval, vector:[1,2,3]});
  if(ts.toCSV() != "2014-01-01T00:00:00+01:00,1\n2014-01-01T00:15:00+01:00,2\n2014-01-01T00:30:00+01:00,3")
    console.error('Timeseries.prototype.toCSV(): failed test: returned ' + ts.toCSV() + ' while expected ' + '2014-01-01T00:00:00+01:00,1\n2014-01-01T00:15:00+01:00,2\n2014-01-01T00:30:00+01:00,3');
