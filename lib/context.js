Fiber(function() {
  ...
  var list = function(search) {
    ...
    var result;
    var fields = { _id: 1 };
    var dbc = db.collection(context.collection);
    dbc.find(query, fields, function (err, data) {
      if(err || !data) result = [];
      else result = data.map(function(d){ return d._id });
      fiber.run();
    });
    Fiber.yield(result);
    return result;
  ...
  var save = function(object) {
    ...
    var result;
    var dbc = db.collection(context.collection);
    dbc.update({ _id: object._id }, { $set: object }, { upsert: true }, function (err, data) {
      result = data;
      fiber.run();
    });
    Fiber.yield(result);
  ...
  var trigger = function(id, variable){
    ...
    dbc.insert({ event: 'update', message: message }, function (err, data) {
      result = data;
      fiber.run();
    });
    Fiber.yield(result);
